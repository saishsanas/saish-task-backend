...
const TaskListScreen: React.FC = () => {
  ...
  // If taskLists is null, undefined or not an array → show loader
  if (!state.taskLists || !Array.isArray(state.taskLists)) {
    return (
        ...
        state.taskLists.map((list) => (
          ...
          >
            <CardBody>
              ...
              <p className="text-sm text-gray-400 mt-2">{list.count} tasks</p>
              <Progress value={list.progress ? list.progress * 100 : 0} className="mt-2" />
            </CardBody>
          </Card>
        ))
      )}
